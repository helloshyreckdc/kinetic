...
int main(int argc, char** argv)
{
    ...
    if(argc != 2)
     {
         cerr << endl << "Usage: rosrun autocalibration  autocalibration     path_to_paramters " << endl;
	 cout<<argc<<endl;
         ros::shutdown();
         return 1;
     }
    ...
    String jointstateFilePath=pd.getData("JOINTSTATEFILEPATH");
    initPoint();
/************************************************************************************/
  ...
  Mat jointstate;
  FileStorage fs1(jointstateFilePath,FileStorage::READ);
  fs1["jointstate"]>>jointstate;
   cout<<jointstate<<endl;
  fs1.release();
  int numOfPose=jointstate.rows;
 ...
 {
Mat pose =jointstate.row(i);
 moveitSuccess= movetoPoseJointstate("manipulator", pose);
     //get board poseture
...
//  //the robot move correctly and find marker pose
  ...
  FileStorage fs2("/home/mj/image/newresult.xml",FileStorage::WRITE);
...
/******************************************************************************/
   ...
   FileStorage fs3("/home/mj/image/end2board.xml",FileStorage::WRITE);
   fs3<<"End2board"<<F_real.transform;
   fs3<<"transbase2camera"<<trans_result;
   fs3.release();
	  base_link2camera_link(trans_x);
