...
// control the process of peg_in_hole
//
#include <ros/ros.h>
#include <string>
#include <iostream>
    ...
    while (node.ok()){
        ...
        if(previous_task_id != current_task_id){
            switch(current_task_id){
                ...
                case start_demo:
                    system("gnome-terminal -x rosrun peg_in_hole record __name:=record");
                ...
                case replay_origin:
//                    system(("rosrun peg_in_hole replay_go_to_origin "
//                    + bag_name + " __name:=replay_go_to_origin").c_str());
                    system(("gnome-terminal -x rosrun peg_in_hole replay_go_to_origin "
                            + bag_name + " __name:=replay_go_to_origin").c_str());
                    break;

                case replay:
                    system(("gnome-terminal -x rosrun peg_in_hole replay_traj "+bag_name+" __name:=replay_traj").c_str());
//                    system(("rosrun peg_in_hole replay_traj "+bag_name+" __name:=replay_traj").c_str());
                    break;
