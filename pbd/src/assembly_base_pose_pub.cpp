...
{
	ros::init(argc, argv, "assembly_base_pose_pub");
	ros::NodeHandle node;
	...
	ROS_INFO("PointCloud after downsampling: %d data points. ", cloud_down_sampled->width * cloud_down_sampled->height);
	...
	ROS_INFO("PointCloud after removing outliers: %d data points. ", cloud_outlier_removed->width * cloud_outlier_removed->height);
